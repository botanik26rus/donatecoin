...
//Заменить
uint256 hashGenesisBlock("0x0dae63aa06a95f8b00e714efc12a23caa9fee795b8dd62e5e27e3039b5b0fa78");
...
const string strMessageMagic = "DonateCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight < 17280) // нет вознаграждения за блок в течение первых 3 дней
        nSubsidy = 0;
    if(nHeight > 10519200) // без блока после 5 лет
        nSubsidy = 0;
...
static const int64 nTargetSpacing = 15; // Блок каждые 15 секунд (не точно)
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal donatecoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DonateCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew){
	...
	// vMerkleTree: 5a2e19825b
        ...
        // Genesis block
        const char* pszTimestamp = "Donatcoin create date 20122017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
	//Заменить
        block.nTime    = 1513802689;
        block.nBits    = 0x1e0ffff0;
	//Заменить
        block.nNonce   = 2712399;
	...
	//Заменить
        assert(block.hashMerkleRoot == uint256("0x1af6f43ab06181595b8cd32146062ca11f288d55037499898901c9e6f665b9b6"));

        // Если хеш блока генезиса не совпадает, то генерировать новый генезис генезис.
	//Перед началом создания генезита True после False
